...
def UCS(width, height, start, target, config):
    ...
    while len(frontier)>0:
        parentCost = min(frontierCost)    #choose the lowest-cost node in frontier
        node = frontier.pop(frontierCost.index(parentCost))    #remove this node
        frontierCost.pop(frontierCost.index(parentCost))    #remove the cost of this node
        explored.append(node)
        if node[0]>0:
            # Go UP
            child = [node[0]-1,node[1]]
            childHash = float(str(child[0])+'.'+str(child[1]))
            # sometimes we should update forntier
            if child not in explored and config[child[0]][child[1]]!=-1:
                if child==target:
                    ...
                    parent[childHash] = node
                    break
                childCost = config[child[0]][child[1]] + parentCost    # record the cost
                # Child not in frotier, normal action
                if child not in frontier:
                    frontier.append(child)
                    frontierCost.append(childCost)
                    parent[childHash] = node
                # Child in frotier and has a smaller cost
                elif childCost < frontierCost[frontier.index(child)]:
                    frontierCost[frontier.index(child)] = childCost     #update cost
                    parent[childHash] = node    #update path
        if node[0]<height-1:
            # Go DOWN
            child = [node[0]+1,node[1]]
            childHash = float(str(child[0])+'.'+str(child[1]))
            if child not in explored and config[child[0]][child[1]]!=-1:
                if child==target:
                    ...
                    parent[childHash] = node
                    break
                childCost = config[child[0]][child[1]] + parentCost
                if child not in frontier:
                    frontier.append(child)
                    frontierCost.append(childCost)
                    parent[childHash] = node
                elif childCost < frontierCost[frontier.index(child)]:
                    frontierCost[frontier.index(child)] = childCost
                    parent[childHash] = node
        if node[1]>0:
            # Go LEFT
            child = [node[0],node[1]-1]
